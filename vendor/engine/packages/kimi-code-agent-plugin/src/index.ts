...
// Declarative hook configuration passed through the engine's plugin options
// slot (config.plugin = [[pluginSpec, options]]). Swift never ships code here
// — only these four typed knobs, generated by KimiHookConfigStore and surfaced
// through KimiHookSettingsView. Keeping the shape flat and JSON-serializable
// means the Swift side never needs to know about opencode's Hooks interface.
export type KimiHookOptions = {
  systemPromptRules?: string[]
  permissionOverrides?: Record<string, 'allow' | 'deny'>
  webFetchAllowedDomains?: string[]
  toolOutputCharLimits?: Record<string, number>
}

export function applySystemPromptRules(rules: string[] | undefined, output: { system: string[] }): void {
  if (!rules?.length) return
  output.system.push(...rules)
}

export function applyPermissionOverride(
  overrides: Record<string, 'allow' | 'deny'> | undefined,
  input: { type: string },
  output: { status: 'ask' | 'deny' | 'allow' },
): void {
  const override = overrides?.[input.type]
  if (override) output.status = override
}

export function enforceWebFetchAllowlist(
  allowedDomains: string[] | undefined,
  input: { tool: string },
  output: { args: any },
): void {
  if (input.tool !== 'webfetch' || !allowedDomains?.length) return
  const host = new URL(String(output.args?.url ?? '')).hostname.toLowerCase()
  const allowed = allowedDomains.some((domain) => {
    const normalized = domain.trim().toLowerCase()
    return host === normalized || host.endsWith(`.${normalized}`)
  })
  if (!allowed) throw new Error(`webfetch target "${host}" is not in the allowed domain list`)
}
...
export function truncateToolOutput(
  ...
  if (!limit || output.output.length <= limit) return
  output.output = `${output.output.slice(0, limit)}\n... [output truncated: exceeded ${limit} character limit]`
}

const PluginServer: Plugin = async (_input, rawOptions) => {
  const options = (rawOptions ?? {}) as KimiHookOptions
  return {
  tool: {
    websearch: tool({
  ...
  async 'experimental.chat.system.transform'(_input, output) {
    applySystemPromptRules(options.systemPromptRules, output)
  },
  async 'permission.ask'(input, output) {
    applyPermissionOverride(options.permissionOverrides, input, output)
  },
  async 'tool.execute.before'(input, output) {
    enforceWebFetchAllowlist(options.webFetchAllowedDomains, input, output)
  },
  async 'tool.execute.after'(input, output) {
    truncateToolOutput(options.toolOutputCharLimits, input, output)
  },
  }
}
